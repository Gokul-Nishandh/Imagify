...
const Login = () => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 z-10 bacdrop-blur-sm bg-black/30 flex justify-center items-center">
        <motion.form onSubmit={onSubmitHandler}initial={{ opacity: 0.2, y: 50 }}
      ...
      viewport={{ once: true }} className="relative bg-white p-10 rounded-xl text-slate-500">
            ...
            <p className="text-sm">Welcome back ! Please sign in to continue</p>
             {state != "Login" && <div className="border px-6 py-2 flex items-center gap-2 rounded-full mt-5">
                ...
                <input className="outline-none text-sm"type="text" onChange={e =>setName(e.target.value)} value={name} placeholder="Full Name" required />  
             </div>}
